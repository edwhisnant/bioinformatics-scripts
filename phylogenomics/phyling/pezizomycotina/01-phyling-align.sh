...
#SBATCH --output=/hpc/group/bio1/ewhisnant/comp-genomics/compare/logs/phyling/pezizomycotina-align.out
#SBATCH --error=/hpc/group/bio1/ewhisnant/comp-genomics/compare/logs/phyling/pezizomycotina-align.err
...
# This script will need to be rewritten for several key things:

# 1. The .protein files from the lecanoromycetes and the pezizomycotina need to be copied into
# a single directory. This is needed for the input for phyling

# 2. Will need to run hmmsearch and alignment for all genomes in the dataset

# 115 from Lecanoromycetes
# 278 or 279 from the Pezizomycotina
# Total 393 or 394 genomes
...
find "${LECAN_IN_DIR}" -path "*/annotate_results/*proteins.fa" -exec cp {} ${TEMP_DIR} \; # this works
find "${PEZIZO_IN_DIR}" -path "*/annotate_results/*proteins.fa" -exec cp {} ${TEMP_DIR} \; # this works

# Report the number of files copied
echo "$(ls ${TEMP_DIR} | wc -l)" "protein FASTA files copied to ${TEMP_DIR}"
